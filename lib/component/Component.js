...
Component.prototype = {
  ...
  getLocalDependenciesTree: function (callback) {
    ...
    function getDependencies(owner, name) {
      var componentJson = path.join(fecom.config.dir, name, 'component.json');
      var node = {};
      ...
      node.dependencies = dependenciesMap.map(function (semantic) {
        return {
    ...
    var getChildren = function (tree) {
      ...
      var node = getDependencies(parsed.owner, parsed.name);
      tree.props = fecom.parse(node.name);
      tree.subNodes = node.dependencies;
